...
using SSEdge = AATriangulator::SSEdge;
using EventList = AATriangulator::EventList;
using Event = AATriangulator::Event;
using EventComparator = AATriangulator::EventComparator;
...
struct AATriangulator::EventList : EventPQ {
  EventList(EventComparator comparison) : EventPQ(comparison) {
  }
};

void AATriangulator::makeEvent(SSEdge* e, EventList* events) const {
  ...
  if (bisector1.intersect(bisector2, &p, &alpha)) {
    TESS_LOG("found edge event for %g, %g (original %g -> %g), "
        ...
        alpha);
    e->fEvent = new Event(e, p, alpha);
    events->push(e->fEvent);
  }
}

void AATriangulator::makeEvent(SSEdge* edge, Vertex* v, SSEdge* other, Vertex* dest,
  ...
  line.fC = -(dest->fPoint.x * line.fA  + dest->fPoint.y * line.fB);
  ...
  if (line.intersect(bisector.fLine, &p) && !c.sweep_lt(p, top->fPoint) &&
    ...
    edge->fEvent = new Event(edge, p, alpha);
    events->push(edge->fEvent);
  }
}

void AATriangulator::connectPartners(VertexList* mesh, const Comparator& c) {
...
void AATriangulator::removeNonBoundaryEdges(const VertexList& mesh) const {
...
static void get_edge_normal(const Edge* e, Point* normal) {
  normal->set(e->fLine.fA,
              e->fLine.fB);
}
...
void AATriangulator::simplifyBoundary(EdgeList* boundary, const Comparator& c) {
...
void AATriangulator::connectSSEdge(Vertex* v, Vertex* dest, const Comparator& c) {
  if (v == dest) {
    return;
  }
...
void AATriangulator::Event::apply(VertexList* mesh, const Comparator& c, EventList* events,
                                    AATriangulator* triangulator) {
  if (!fEdge) {
    return;
  }
  ...
  SSVertex* ssv = new SSVertex(dest);
...
bool AATriangulator::collapseOverlapRegions(VertexList* mesh, const Comparator& c,
  ...
  for (Vertex* v = mesh->fHead; v != nullptr; v = v->fNext) {
    ...
    for (Edge* e = v->fLastEdgeAbove; e && e != leftEnclosingEdge;) {
      ...
      if (leftOverlap && rightOverlap) {
        ...
        if (!ssPrev) {
          ssPrev = ssVertices[prevVertex] = new SSVertex(prevVertex);
        }
        SSVertex* ssNext = ssVertices[nextVertex];
        if (!ssNext) {
          ssNext = ssVertices[nextVertex] = new SSVertex(nextVertex);
        }
        SSEdge* ssEdge = new SSEdge(e, ssPrev, ssNext);
        ...
        if (!isOuterBoundary) {
          ...
          e->fWinding = std::copysignf(e->fType == EdgeType::kInner ? 2 : 1,
                                         e->fWinding);
        }
      }
      e = prev;
...
void AATriangulator::strokeBoundary(EdgeList* boundary, VertexList* innerMesh,
                                      const Comparator& c) {
  TESS_LOG("\nstroking boundary\n");
  ...
  for (Edge* e = boundary->fHead; e != nullptr; e = e->fRight) {
    ...
    if (!prevEdge->fLine.nearParallel(e->fLine) && prevInner.intersect(inner, &innerPoint) &&
      ...
      if (cosAngle < -kCosMiterAngle) {
        ...
        Line tangent(v->fPoint, v->fPoint + Point::Make(bisector.fA, bisector.fB));
        ...
        innerTangent.fC -= 0.5;
        outerTangent.fC += 0.5;
        Point innerPoint1, innerPoint2, outerPoint1, outerPoint2;
        if (prevNormal.cross(normal) > 0) {
          ...
          Line prevTangent(prevV->fPoint,
                           prevV->fPoint + Point::Make(prevOuter.fA, prevOuter.fB));
          Line nextTangent(nextV->fPoint,
                           nextV->fPoint + Point::Make(outer.fA, outer.fB));
          ...
          Line prevTangent(prevV->fPoint,
                           prevV->fPoint + Point::Make(prevInner.fA, prevInner.fB));
          Line nextTangent(nextV->fPoint,
                           nextV->fPoint + Point::Make(inner.fA, inner.fB));
        ...
        Vertex* innerVertex1 = new Vertex(innerPoint1, 255);
        Vertex* innerVertex2 = new Vertex(innerPoint2, 255);
        Vertex* outerVertex1 = new Vertex(outerPoint1, 0);
        Vertex* outerVertex2 = new Vertex(outerPoint2, 0);
        ...
        Vertex* innerVertex = new Vertex(innerPoint, 255);
        Vertex* outerVertex = new Vertex(outerPoint, 0);
...
void AATriangulator::extractBoundary(EdgeList* boundary, Edge* e) const {
  TESS_LOG("\nextracting boundary\n");
...
void AATriangulator::extractBoundaries(const VertexList& inMesh, VertexList* innerVertices,
                                         const Comparator& c) {
...
std::tuple<Poly*, bool> AATriangulator::tessellate(const VertexList& mesh, const Comparator& c) {
  ...
  if (was_complex) {
    ...
    return Triangulator::tessellate(aaMesh, c);
  } else {
    TESS_LOG("no complex polygons; taking fast path\n");
    return Triangulator::tessellate(innerMesh, c);
  }
}

std::vector<float> AATriangulator::polysToAATriangles(Poly* polys) const {
  int64_t count64 = CountPoints(polys, PathFillType::Winding);
  ...
  polysToTriangles(polys, PathFillType::Winding, &data);
