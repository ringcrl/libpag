...
static float LengthSqd(const Point& pt) {
  return DotProduct(pt, pt);
}

static float DistanceToSqd(const Point& pt, const Point& a) {
  float dx = pt.x - a.x;
  float dy = pt.y - a.y;
  return dx * dx + dy * dy;
}

static float DistanceToLineSegmentBetweenSqd(const Point& pt, const Point& a,
                                                      const Point& b) {
...
using EdgeType = Triangulator::EdgeType;
using Vertex = Triangulator::Vertex;
using VertexList = Triangulator::VertexList;
using Line = Triangulator::Line;
using Edge = Triangulator::Edge;
using EdgeList = Triangulator::EdgeList;
using Poly = Triangulator::Poly;
using MonotonePoly = Triangulator::MonotonePoly;
using Comparator = Triangulator::Comparator;
...
typedef bool (*CompareFunc)(const Point& a, const Point& b);

static bool sweep_lt_horiz(const Point& a, const Point& b) {
  return a.x < b.x || (a.x == b.x && a.y > b.y);
}

static bool sweep_lt_vert(const Point& a, const Point& b) {
  return a.y < b.y || (a.y == b.y && a.x < b.x);
}

bool Triangulator::Comparator::sweep_lt(const Point& a, const Point& b) const {
...
static inline void emit_vertex(Vertex* v,
  ...
  data->push_back(v->fPoint.x);
  data->push_back(v->fPoint.y);
...
static void emit_triangle(Vertex* v0, Vertex* v1, Vertex* v2,
                                         bool emitCoverage, std::vector<float>* data) {
...
void Triangulator::VertexList::insert(Vertex* v, Vertex* prev, Vertex* next) {
  list_insert<Vertex, &Vertex::fPrev, &Vertex::fNext>(v, prev, next, &fHead, &fTail);
}

void Triangulator::VertexList::remove(Vertex* v) {
...
static inline void round(Point* p) {
  p->x = std::round(p->x * 4.0f) * 0.25f;
  p->y = std::round(p->y * 4.0f) * 0.25f;
}
...
static inline float double_to_clamped_scalar(double d) {
  ...
  static const double kMaxLimit = (double) 3.402823466e+38f;
...
bool Triangulator::Line::intersect(const Line& other, Point* point) const {
...
static bool edge_line_needs_recursion(const Point& p0, const Point& p1) {
...
static bool recursive_edge_intersect(const Line& u, Point u0, Point u1,
                                     const Line& v, Point v0, Point v1,
                                     Point* p, double* s, double* t) {
  ...
  if (std::min(u0.x, u1.x) > std::max(v0.x, v1.x) ||
      ...
      std::min(u0.y, u1.y) > std::max(v0.y, v1.y) ||
      std::max(u0.y, u1.y) < std::min(v0.y, v1.y)) {
    return false;
  }
  ...
  *t = tNumer / denom;
  ...
  if (!uNeedsSplit && !vNeedsSplit) {
    ...
    if (uNeedsSplit) {
      Point uM = {(float) (0.5 * u0.x + 0.5 * u1.x),
                    (float) (0.5 * u0.y + 0.5 * u1.y)};
      sScale = 0.5;
      if (*s >= 0.5) {
        u0 = uM;
        sShift = 0.5;
    ...
    if (vNeedsSplit) {
      Point vM = {(float) (0.5 * v0.x + 0.5 * v1.x),
                    (float) (0.5 * v0.y + 0.5 * v1.y)};
      tScale = 0.5;
      if (*t >= 0.5) {
        v0 = vM;
        tShift = 0.5;
...
bool Triangulator::Edge::intersect(const Edge& other, Point* p, uint8_t* alpha) const {
...
void Triangulator::EdgeList::insert(Edge* edge, Edge* prev, Edge* next) {
  list_insert<Edge, &Edge::fLeft, &Edge::fRight>(edge, prev, next, &fHead, &fTail);
}

void Triangulator::EdgeList::remove(Edge* edge) {
  TESS_LOG("removing edge %g -> %g\n", edge->fTop->fID, edge->fBottom->fID);
  list_remove<Edge, &Edge::fLeft, &Edge::fRight>(edge, &fHead, &fTail);
}

void Triangulator::MonotonePoly::addEdge(Edge* edge) {
...
void Triangulator::emitMonotonePoly(const MonotonePoly* monotonePoly,
  ...
  while (v != vertices.fTail) {
    ...
    Vertex* next = v->fNext;
    if (count == 3) {
      emitTriangle(prev, curr, next, monotonePoly->fWinding, data);
      return ;
    }
...
void Triangulator::emitTriangle(
...
Poly* Triangulator::Poly::addEdge(Edge* e, Side side, Triangulator* tri) {
...
void Triangulator::emitPoly(const Poly* poly, std::vector<float>* data) const {
  if (poly->fCount < 3) {
    return;
  }
  TESS_LOG("emit() %d, size %d\n", poly->fID, poly->fCount);
  for (MonotonePoly* m = poly->fHead; m != nullptr; m = m->fNext) {
    emitMonotonePoly(m, data);
  }
}
...
Poly* Triangulator::makePoly(Poly** head, Vertex* v, int winding) const {
  Poly* poly = new Poly(v, winding);
  poly->fNext = *head;
  *head = poly;
  return poly;
}

void Triangulator::appendPointToContour(const Point& p, VertexList* contour) const {
  Vertex* v = new Vertex(p, 255);
#if TRIANGULATOR_LOGGING
...
static float quad_error_at(const pk::SkPoint pts[3], float t, float u) {
  pk::SkQuadCoeff quad(pts);
  auto p0 = pk::to_point(quad.eval(t - 0.5f * u));
  auto mid = pk::to_point(quad.eval(t));
  auto p1 = pk::to_point(quad.eval(t + 0.5f * u));
  if (!p0.isFinite() || !mid.isFinite() || !p1.isFinite()) {
    return 0;
  }
  return pk::SkPointPriv::DistanceToLineSegmentBetweenSqd(mid, p0, p1);
}

// We guarantee that no quad or cubic will ever produce more than this many points
static const int kMaxPointsPerCurve = 1 << 10;

void Triangulator::appendQuadraticToContour(const Point pts[3], float toleranceSqd,
  ...
  pk::SkQuadCoeff quad(skPts);
  pk::Sk2s aa = quad.fA * quad.fA;
  float denom = 2.0f * (aa[0] + aa[1]);
  pk::Sk2s ab = quad.fA * quad.fB;
  ...
  while (nPoints < kMaxPointsPerCurve) {
    u = 1.0f / nPoints;
    if (quad_error_at(skPts, t, u) < toleranceSqd) {
      break;
    }
    nPoints++;
  }
  for (int j = 1; j <= nPoints; j++) {
    auto skPoint = pk::to_point(quad.eval(j * u));
    auto p = Point::Make(skPoint.fX, skPoint.fY);
    appendPointToContour(p, contour);
  }
}


void Triangulator::generateCubicPoints(const Point& p0, const Point& p1, const Point& p2,
                                         const Point& p3, float tolSqd, VertexList* contour,
                                         int pointsLeft) const {
  float d1 = DistanceToLineSegmentBetweenSqd(p1, p0, p3);
  float d2 = DistanceToLineSegmentBetweenSqd(p2, p0, p3);
  ...
  const Point q[] = {
      { SkScalarAve(p0.x, p1.x), SkScalarAve(p0.y, p1.y) },
      { SkScalarAve(p1.x, p2.x), SkScalarAve(p1.y, p2.y) },
      { SkScalarAve(p2.x, p3.x), SkScalarAve(p2.y, p3.y) }
  };
  const Point r[] = {
      { SkScalarAve(q[0].x, q[1].x), SkScalarAve(q[0].y, q[1].y) },
      { SkScalarAve(q[1].x, q[2].x), SkScalarAve(q[1].y, q[2].y) }
  };
  const Point s = { SkScalarAve(r[0].x, r[1].x), SkScalarAve(r[0].y, r[1].y) };
  pointsLeft >>= 1;
...
static  void toQuad(Rect rect, Point quad[4]) {
  quad[0].set(rect.left, rect.top);
  quad[1].set(rect.right, rect.top);
  quad[2].set(rect.right, rect.bottom);
  quad[3].set(rect.left, rect.bottom);
}

static const float kMinCurveTol = 0.0001f;

static float tolerance_to_wangs_precision(float srcTol) {
  // You should have called scaleToleranceToSrc, which guarantees this
  DEBUG_ASSERT(srcTol >= kMinCurveTol);
...
template<int N> using vec = pk::skvx::Vec<N, float>;
using float2 = vec<2>;
using float4 = vec<4>;

static float cubic_pow4(float precision, const Point pts[]) {
...
// int.
int cubic_log2(float precision,
                  const Point pts[]) {
  // nextlog16(x) == ceil(log2(sqrt(sqrt(x))))
  return nextlog16(cubic_pow4(precision, pts));
}

uint32_t max_bezier_vertices(uint32_t chopCount) {
  static constexpr uint32_t kMaxChopsPerCurve = 10;
  static_assert((1 << kMaxChopsPerCurve) == kMaxPointsPerCurve);
  return 1 << std::min(chopCount, kMaxChopsPerCurve);
}

uint32_t cubicPointCount(const Point points[], float tol) {
  return max_bezier_vertices(cubic_log2(
      tolerance_to_wangs_precision(tol), points));
}

// Stage 1: convert the input path to a set of linear contours (linked list of Vertices).

void Triangulator::pathToContours(float tolerance, const Rect& clipBounds,
  ...
  *isLinear = true;
  ...
  if (fPath.isInverseFillType()) {
    Point quad[4];
    toQuad(clipBounds, quad);
    for (int i = 3; i >= 0; i--) {
      appendPointToContour(quad[i], contours);
    }
    contour++;
  }
  SkAutoConicToQuads converter;
  fPath.decompose([&](PathVerb verb, const Point pts[4], void* ){
    switch (verb) {
      case PathVerb::Move:
        if (contour->fHead) {
          contour++;
      ...
      case PathVerb::Cubic: {
        *isLinear = false;
        ...
        int pointsLeft = cubicPointCount(pts, tolerance);
...
static inline bool apply_fill_type(PathFillType fillType, int winding) {
  switch (fillType) {
...
bool Triangulator::applyFillType(int winding) const {
  return apply_fill_type(fPath.getFillType(), winding);
}

static inline bool apply_fill_type(PathFillType fillType, Poly* poly) {
  return poly && apply_fill_type(fillType, poly->fWinding);
}

MonotonePoly* Triangulator::allocateMonotonePoly(Edge* edge, Side side, int winding) {
  ++fNumMonotonePolys;
  return new MonotonePoly(edge, side, winding);
}

Edge* Triangulator::allocateEdge(Vertex* top, Vertex* bottom, int winding, EdgeType type) {
  ++fNumEdges;
  return new Edge(top, bottom, winding, type);
}

Edge* Triangulator::makeEdge(Vertex* prev, Vertex* next, EdgeType type,
                               const Comparator& c) {
...
void Triangulator::FindEnclosingEdges(Vertex* v, EdgeList* edges, Edge** left, Edge** right) {
...
void Triangulator::Edge::insertAbove(Vertex* v, const Comparator& c) {
...
void Triangulator::Edge::insertBelow(Vertex* v, const Comparator& c) {
  if (fTop->fPoint == fBottom->fPoint ||
      c.sweep_lt(fBottom->fPoint, fTop->fPoint)) {
    return;
  }
...
void Triangulator::Edge::disconnect() {
...
void Triangulator::setTop(Edge* edge, Vertex* v, EdgeList* activeEdges, Vertex** current,
                            const Comparator& c) const {
  remove_edge_below(edge);
  if (fCollectBreadcrumbTriangles) {
    fBreadcrumbList.append(edge->fTop->fPoint, edge->fBottom->fPoint, v->fPoint,
                           edge->fWinding);
  }
  edge->fTop = v;
  edge->recompute();
...
void Triangulator::setBottom(Edge* edge, Vertex* v, EdgeList* activeEdges, Vertex** current,
                               const Comparator& c) const {
...
void Triangulator::mergeEdgesAbove(Edge* edge, Edge* other, EdgeList* activeEdges,
  ...
  if (coincident(edge->fTop->fPoint, other->fTop->fPoint)) {
    TESS_LOG("merging coincident above edges (%g, %g) -> (%g, %g)\n",
             edge->fTop->fPoint.x, edge->fTop->fPoint.y,
             edge->fBottom->fPoint.x, edge->fBottom->fPoint.y);
...
void Triangulator::mergeEdgesBelow(Edge* edge, Edge* other, EdgeList* activeEdges,
                                     Vertex** current, const Comparator& c) const {
...
void Triangulator::mergeCollinearEdges(Edge* edge, EdgeList* activeEdges, Vertex** current,
                                         const Comparator& c) const {
  for (;;) {
...
bool Triangulator::splitEdge(Edge* edge, Vertex* v, EdgeList* activeEdges, Vertex** current,
                               const Comparator& c) {
  if (!edge->fTop || !edge->fBottom || v == edge->fTop || v == edge->fBottom) {
    return false;
  }
  TESS_LOG("splitting edge (%g -> %g) at vertex %g (%g, %g)\n",
           edge->fTop->fID, edge->fBottom->fID, v->fID, v->fPoint.x, v->fPoint.y);
  Vertex* top;
  Vertex* bottom;
...
bool Triangulator::intersectEdgePair(Edge* left, Edge* right, EdgeList* activeEdges,
...
Edge* Triangulator::makeConnectingEdge(Vertex* prev, Vertex* next, EdgeType type,
...
void Triangulator::mergeVertices(Vertex* src, Vertex* dst, VertexList* mesh,
                                   const Comparator& c) const {
  TESS_LOG("found coincident verts at %g, %g; merging %g into %g\n",
           src->fPoint.x, src->fPoint.y, src->fID, dst->fID);
...
Vertex* Triangulator::makeSortedVertex(const Point& p, uint8_t alpha, VertexList* mesh,
...
// imposed by 'c').
static Point clamp(Point p, Point min, Point max, const Comparator& c) {
  if (c.fDirection == Comparator::Direction::kHorizontal) {
    ...
    return {SkTPin(p.x, min.x, max.x),
            min.y < max.y ? SkTPin(p.y, min.y, max.y)
                            : SkTPin(p.y, max.y, min.y)};
  } else {
    // And with vertical sorting, we know Y's relation but not necessarily X's.
    return {min.x < max.x ? SkTPin(p.x, min.x, max.x)
                            : SkTPin(p.x, max.x, min.x),
            SkTPin(p.y, min.y, max.y)};
  }
}

void Triangulator::computeBisector(Edge* edge1, Edge* edge2, Vertex* v) const {
  ...
  if (line1.intersect(line2, &p)) {
    ...
    v->fPartner = new Vertex(p, alpha);
    TESS_LOG("computed bisector (%g,%g) alpha %d for vertex %g\n", p.x, p.y, alpha, v->fID);
  }
}

bool Triangulator::checkForIntersection(Edge* left, Edge* right, EdgeList* activeEdges,
...
void Triangulator::sanitizeContours(VertexList* contours, int contourCnt) const {
...
bool Triangulator::mergeCoincidentVertices(VertexList* mesh, const Comparator& c) const {
  if (!mesh->fHead) {
    return false;
  }
  bool merged = false;
...
void Triangulator::buildEdges(VertexList* contours, int contourCnt, VertexList* mesh,
                                const Comparator& c) {
  for (VertexList* contour = contours; contourCnt > 0; --contourCnt, ++contour) {
    Vertex* prev = contour->fTail;
    for (Vertex* v = contour->fHead; v;) {
...
void Triangulator::SortedMerge(VertexList* front, VertexList* back, VertexList* result,
                                 const Comparator& c) {
  if (c.fDirection == Comparator::Direction::kHorizontal) {
  ...
  for (Vertex* v = fHead; v; v = v->fNext) {
    ...
    if (Vertex* p = v->fPartner) {
      TESS_LOG(", partner %g (%g, %g) alpha %d\n",
               p->fID, p->fPoint.x, p->fPoint.y, p->fAlpha);
    } else {
...
Triangulator::SimplifyResult Triangulator::simplify(VertexList* mesh,
                                                        const Comparator& c) {
...
std::tuple<Poly*, bool> Triangulator::tessellate(const VertexList& vertices, const Comparator&) {
...
void Triangulator::contoursToMesh(VertexList* contours, int contourCnt, VertexList* mesh,
                                    const Comparator& c) {
#if TRIANGULATOR_LOGGING
...
void Triangulator::SortMesh(VertexList* vertices, const Comparator& c) {
...
std::tuple<Poly*, bool> Triangulator::contoursToPolys(VertexList* contours, int contourCnt) {
  const Rect& pathBounds = fPath.getBounds();
...
void Triangulator::polysToTriangles(Poly* polys,
                                                     PathFillType overrideFillType,
                                                     std::vector<float>* data) const {
...
static int get_contour_count(const Path& path, float ) {
  ...
  bool first = true;
  path.decompose([&](PathVerb verb, const Point [4], void* ) {
    switch (verb) {
      case PathVerb::Move:
        if (!first) {
          ++contourCnt;
...
static inline bool SkPathFillType_IsInverse(PathFillType ft) {
  return (static_cast<int>(ft) & 2) != 0;
}

std::tuple<Poly*, bool> Triangulator::pathToPolys(float tolerance, const Rect& clipBounds, bool* isLinear) {
...
int64_t Triangulator::CountPoints(Poly* polys, PathFillType overrideFillType) {
  int64_t count = 0;
  for (Poly* poly = polys; poly; poly = poly->fNext) {
...
std::vector<float> Triangulator::polysToTriangles(Poly* polys) const {
...
//  size_t vertexStride = sizeof(Point);
//  if (fEmitCoverage) {
...
//  }
  ...
  polysToTriangles(polys, fPath.getFillType(), &points);
//  for (size_t i = 0; i < points.size(); i+=2) {
//    LOGI("{x: %f, y: %f}", points[i], points[i + 1]);
////    LOGI("{x: %f, y: %f, a: %f}", points[i], points[i + 1], points[i + 2]);
//  }
  return points;
}
}  // namespace tgfx
