...
class Triangulator {
 public:
  ...
  static std::vector<float> PathToTriangles(const Path& path, float tolerance, const Rect& clipBounds,
                             bool* isLinear) {
    if (path.isEmpty()) {
      return {};
    }
    Triangulator triangulator(path);
    auto [ polys, success ] = triangulator.pathToPolys(tolerance, clipBounds, isLinear);
    if (!success) {
      return {};
    }
    return triangulator.polysToTriangles(polys);
  }

 protected:
  Triangulator(Path  path) : fPath(std::move(path)) {}
  ...
  void pathToContours(float tolerance, const Rect& clipBounds, VertexList* contours,
                      bool* isLinear) const;
  ...
  void emitTriangle(Vertex* prev, Vertex* curr, Vertex* next, int winding,
                                   std::vector<float>* data) const;
  void emitPoly(const Poly*, std::vector<float>* data) const;

  Poly* makePoly(Poly** head, Vertex* v, int winding) const;
  void appendPointToContour(const Point& p, VertexList* contour) const;
  void appendQuadraticToContour(const Point[3], float toleranceSqd,
                                VertexList* contour) const;
  void generateCubicPoints(const Point&, const Point&, const Point&, const Point&,
                           float tolSqd, VertexList* contour, int pointsLeft) const;
  ...
  bool intersectEdgePair(Edge* left, Edge* right, EdgeList* activeEdges, Vertex** current,
                         const Comparator&);
  Vertex* makeSortedVertex(const Point&, uint8_t alpha, VertexList* mesh, Vertex* reference,
                           const Comparator&) const;
  ...
  std::tuple<Poly*, bool> pathToPolys(float tolerance, const Rect& clipBounds,
                                      bool* isLinear);
  static int64_t CountPoints(Poly* polys, PathFillType overrideFillType);
  std::vector<float> polysToTriangles(Poly*) const;
  ...
  class BreadcrumbTriangleList {
   public:
    struct Node {
      Node(Point a, Point b, Point c) : fPts{a, b, c} {}
      Point fPts[3];
      Node* fNext = nullptr;
    };
    ...
    void append(Point a, Point b, Point c, int winding) {
...
struct Triangulator::Vertex {
  Vertex(const Point& point, uint8_t alpha)
...
struct Triangulator::VertexList {
...
struct Triangulator::Line {
  ...
  double dist(const Point& p) const { return fA * p.x + fB * p.y + fC; }
  ...
  bool intersect(const Line& other, Point* point) const;
  double fA, fB, fC;
};
...
struct Triangulator::Edge {
  ...
  void disconnect();
  bool intersect(const Edge& other, Point* p, uint8_t* alpha = nullptr) const;
};

struct Triangulator::EdgeList {
...
struct Triangulator::MonotonePoly {
...
struct Triangulator::Poly {
  Poly(Vertex* v, int winding);

  Poly* addEdge(Edge* e, Side side, Triangulator*);
